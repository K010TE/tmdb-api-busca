const token = '' //INSIRA A SUA API KEY ENTRE AS ASPAS DA CONSTANTE TOKEN 
...
const App = () => {
  ...
  const handleSearch = async () => {
    const apiKey = token;
    const searchUrl = `https://api.themoviedb.org/3/search/movie?query=${encodeURIComponent(movieName)}&api_key=${apiKey}`;

    try {
      const searchResponse = await axios.get(searchUrl);
      const movieDetailsPromises = searchResponse.data.results.map(async (movie) => {
        ...
        return {
          ...
          country: movieDetails.production_countries.length > 0 ? movieDetails.production_countries[0].name : 'N/A',
          ...
          <ul className="movie-list">
            {currentResults.map((movie, index) => (
              <li key={index} className="movie-item">
                <img
                  src={movie.poster !== 'N/A' ? movie.poster : '/generic-poster.png'}
